...
def test_agendamento_executa_imediatamente():
    ...
    with (
        ...
        patch("alertavida.scheduler.BackgroundScheduler", return_value=scheduler_mock),
        patch("alertavida.scheduler.datetime") as mock_datetime,
        patch("alertavida.scheduler.time.sleep", side_effect=KeyboardInterrupt),
        patch("alertavida.scheduler.sys.exit"),
    ):
...
def test_keyboard_interrupt_encerra_limpo():
    ...
    with (
        ...
        patch("alertavida.scheduler.BackgroundScheduler", return_value=scheduler_mock),
        patch("alertavida.scheduler.time.sleep", side_effect=KeyboardInterrupt),
        patch("alertavida.scheduler.sys.exit") as mock_exit,
    ):
        scheduler.agendar_ingestao()
