...
- **Parte B — CemadenSource como DataSource + Orquestrador isolado (após A.1 + A.2):** subdividida em três sub-partes encadeadas, cada uma com commit, CI verde e recap próprios:
  ...
  - **B.2.a — Orquestrador isolado em `ingestion/orquestrador.py` ✅ CONCLUÍDA 19/05/2026**: entregue em três commits encadeados (B.2.a + hardening-1 + hardening-2), todos com CI verde em Ubuntu + Windows:
  ...
  - **B.2.b — `monitor.py` reduzido a entrypoint puro 🔒 BLOQUEADA**: refator destrutivo. `monitor.py` perde `executar_ingestao` (migrada para `ingestion/orquestrador.py` em B.2.a) e fica como entrypoint (~30 linhas): parse args, configura logging, instancia `CemadenSource()`, chama `executar_ingestao([source])`, formata e imprime `RelatorioIngestao`, encerra. Remove as 3 comparações `e.tipo == "AlertaX"` (linhas 57-61) — código morto pós-B.2.a. `scheduler.py` ajusta import de `executar_ingestao` para `from alertavida.ingestion.orquestrador import executar_ingestao`. Testes existentes de `tests/test_monitor.py` migram para `tests/ingestion/test_orquestrador.py` (já migrados em B.2.a; arquivo antigo pode esvaziar ou ser removido). Próximo: B.2.b.
