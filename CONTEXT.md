...
- **Parte B — CemadenSource como DataSource + Orquestrador isolado (após A.1 + A.2):** subdividida em três sub-partes encadeadas, cada uma com commit, CI verde e recap próprios:
  ...
  - **B.2 — Orquestrador isolado em `ingestion/orquestrador.py`:**
    - Novo módulo `src/alertavida/ingestion/orquestrador.py` com:
    ...
    - `scheduler.py` ajusta imports — passa a importar `executar_ingestao` de `alertavida.ingestion.orquestrador`
    - Testes existentes de `test_monitor.py` migram para `tests/ingestion/test_orquestrador.py` (testam o orquestrador com `FakeDataSource`, sem mocks de rede)
