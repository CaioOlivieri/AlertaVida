...
- **Parte B — CemadenSource como DataSource + Orquestrador isolado (após A.1 + A.2):** subdividida em três sub-partes encadeadas, cada uma com commit, CI verde e recap próprios:
  ...
  - **B.2.a — Orquestrador isolado em `ingestion/orquestrador.py` ✅ CONCLUÍDA 17/05/2026**: entregue em três commits encadeados (B.2.a + hardening-1 + hardening-2), todos com CI verde em Ubuntu + Windows:

    - B.2.a (commit `5044a7d`) — criação aditiva do pacote `ingestion/`. Novo módulo `src/alertavida/ingestion/__init__.py` exportando `executar_ingestao`, `RelatorioFonte`, `RelatorioIngestao`. Novo módulo `src/alertavida/ingestion/orquestrador.py` com `executar_ingestao(sources: Sequence[DataSource], *, agora: datetime | None = None) -> RelatorioIngestao` — orquestra coleta + detecção + persistência por fonte, isola `FalhaDeColeta` por fonte, propaga qualquer outra exceção. `RelatorioFonte` (frozen dataclass com slots) por fonte contém `coletados`, `novos`, `atualizados`, `inalterados`, `descartados`, `falha_coleta: bool`, `coletado_em: datetime | None`, `duracao_segundos: float`, com `__post_init__` validando invariantes (soma fecha quando `falha_coleta=False`; contadores zerados e `coletado_em=None` quando `falha_coleta=True`; `duracao_segundos >= 0`). `RelatorioIngestao` (frozen, slots) agrega `por_fonte: tuple[RelatorioFonte, ...]` + `agora: datetime` (timezone-aware), com `@property total` somando `coletados` por fonte. Nova suíte `tests/ingestion/__init__.py` + `tests/ingestion/test_orquestrador.py` com 14 testes cobrindo caminho feliz unitário/duplo, falha isolada, propagação de exceção não-prevista, `agora` default vs injetado, ordem das fontes preservada, source vazia, total agregado, `duracao_segundos` finita, `coletado_em` propagado, rodada sem fontes. Zero arquivos modificados; 219 testes totais, < 2s.

    - B.2.a-hardening-1 (commit `c77864d`) — refator de Camada 3: introduz `TipoEventoDetectado(StrEnum)` em `src/alertavida/domain/detector.py` com membros `CRIADO="AlertaCriado"`, `ATUALIZADO="AlertaAtualizado"`, `RESOLVIDO="AlertaResolvido"`. Campo `EventoDetectado.tipo` muda de `str` para `TipoEventoDetectado`. `database.py` e `ingestion/orquestrador.py` consomem a StrEnum via comparação `is` (membros de enum são singletons). `events.py` mantém strings cruas nas `bus.subscribe` (decisão arquitetural: EventBus opera sobre `dict` puro, importar StrEnum criaria dependência Infrastructure → Domain que não existia). `monitor.py` mantém 3 comparações `e.tipo == "AlertaX"` (linhas 57-61) — código será removido em B.2.b. Comportamento e dados em disco permanecem idênticos (StrEnum.value bate com a string anterior; INSERT/SELECT em `eventos.tipo` continuam usando strings). 219 testes totais, 0.90s.

    - B.2.a-hardening-2 (commit `059a6d5`) — testes novos e extensão do `FakeDataSource`. Novo classmethod `FakeDataSource.com_rodadas(fonte, rodadas, *, coletado_em_por_rodada=None)` em `tests/fixtures/sources_fake.py` — fabrica fonte que retorna listas diferentes de alertas em chamadas sucessivas de `coletar()`. Implementado via subclasse interna `_FakeDataSourceMultiRodada` (prefixo `_`), preservando construtor original intacto (princípio: dublê de teste estende sem flag booleana, sem mutação retroativa dos 14 testes anteriores). Validações: `RuntimeError("rodadas esgotadas")` se `coletar()` exceder; `ValueError` se `coletado_em_por_rodada` tiver tamanho diferente de `rodadas`. Renomeação: `test_isolamento_de_persistencia` → `test_persistencia_separada_por_fonte` (corpo idêntico; nome anterior prometia isolamento sob falha que ele não provava). 4 testes novos em `tests/ingestion/test_orquestrador.py`: (1) segunda rodada com mesmo `ult_atualizacao` conta como `inalterados`; (2) segunda rodada com `ult_atualizacao` diferente conta como `atualizados`; (3) alerta ausente por 3 rodadas consecutivas emite `AlertaResolvido` no outbox — verificação via `json_extract(payload, '$.fonte')` e `json_extract(payload, '$.cod_alerta')` na tabela `eventos`, **não** via campo no `RelatorioFonte` (resolvidos são fenômeno derivado da ausência, não do batch coletado); (4) persistência de fontes anteriores sobrevive à falha posterior — monkeypatch em `alertavida.ingestion.orquestrador.aplicar_resultado_deteccao` (namespace de USO, não de definição) injetando `sqlite3.OperationalError` na segunda chamada, source A persistida confirmada via SQL, sources B e C ausentes. Helper `_alerta` estendido com kwarg opcional `ult_atualizacao` (não quebra os 14 testes anteriores). 223 testes totais, 1.78s.

    Pendências reconhecidas que B.2.a NÃO resolve (ficam para depois de B.2.b):
    - **Item 7 — `DB_PATH` explícito:** `executar_ingestao` ainda confia no `DB_PATH` global de `database.py` (monkeypatchado pela fixture `db_temporario` nos testes). Em produção, `monitor.py` precisa importar/setar antes de chamar. Item bloqueador para migração Postgres. Candidato natural para prompt dedicado depois de B.2.b fechar.
    - `aplicar_resultado_deteccao(agora: str)` — a fronteira `datetime → str` via `.isoformat()` está no orquestrador. Refator interno trivial, fica em aberto.
...
| Orquestrador separado em `ingestion/orquestrador.py`, não em `monitor.py` (B.2) | `monitor.py` atual viola SRP três vezes: entrypoint + orquestração + lógica CEMADEN. B.1 extrai a lógica CEMADEN para `sources/cemaden.py`; B.2 extrai a orquestração para `ingestion/orquestrador.py`; `monitor.py` fica como entrypoint puro (~30 linhas: argv, logging config, instancia sources, chama executar_ingestao, formata relatório). Alinha com estrutura-alvo documentada em §4 (subpacote `ingestion/` da Camada 1). |
...
| `RelatorioFonte` com invariantes via `__post_init__`, não validação externa | Frozen dataclass com `__post_init__` que valida soma `novos + atualizados + inalterados + descartados == coletados` (quando `falha_coleta=False`), todos contadores zerados e `coletado_em=None` (quando `falha_coleta=True`), e `duracao_segundos >= 0`. Invariantes vivem no tipo, não em variáveis locais frágeis do orquestrador. Construção inválida levanta `ValueError` imediatamente. Substitui contadores soltos da versão anterior de `monitor.py`. |
| `agora_da_rodada` gerado uma vez pelo orquestrador, propagado para todas as fontes | `executar_ingestao(agora: datetime \| None = None)` gera `datetime.now(UTC)` no início se não fornecido e propaga para todas as chamadas de `aplicar_resultado_deteccao`. Garante que duas fontes coletadas na mesma rodada têm o mesmo carimbo em `visto_ultima_vez`, mantendo coerência do contador `rodadas_ausente` do detector. Cada `ResultadoColeta.coletado_em` interno à fonte registra **quando aquela fonte respondeu** — métrica de observabilidade separada, não confundir com watermark da rodada. Padrão "State boundary" em pipelines multi-fonte (Unstructured 2026; *Architecture Patterns with Python*, cap. 8). |
| Orquestrador silencioso (zero `print`, zero `logging`) | `executar_ingestao` retorna `RelatorioIngestao` tipado e cala a boca. Apresentação é responsabilidade do chamador (`monitor.py` em B.2.b, futuro endpoint `/health` na Camada 6). Logging também é apresentação — para sysadmins em vez de usuários. Decidir "logging vs print" agora seria debate que pode esperar a Camada 6 com infra de observabilidade dedicada. SRP: orquestrador produz dados, consumidores apresentam. |
| `TipoEventoDetectado` StrEnum no detector, NOT um refator de enum dos outros tipos do domínio | StrEnum (Python 3.11+) representa universo fechado de tipos de evento produzidos pelo `ChangeDetector` — `CRIADO`, `ATUALIZADO`, `RESOLVIDO`. Type safety em tempo de check (mypy `--strict` pega typo de membro), match/case exaustivo possível, autocompletar no IDE. StrEnum.value bate com string anterior — INSERT/SELECT no banco continuam armazenando `"AlertaCriado"` etc., migração de dados zero. Decisão NÃO se estende retroativamente para `NivelRisco`, `TipoEvento`, `EscopoGeografico`, `FonteClassificacao`: refator de consolidação dos enums do domínio fica como dívida documentada para janela futura (após B.2.b e Item 7 fecharem). |
| `events.py` mantém strings cruas nas `bus.subscribe`, NÃO importa `TipoEventoDetectado` | Decisão revisada em 17/05/2026 durante hardening-1. Análise inicial promoveu StrEnum para `events.py` ("3 strings mágicas viram 3 membros tipados"). Análise crítica antes do commit revelou que o `EventBus` opera sobre `dict` puro — antes do hardening-1, `events.py` não dependia do domínio. Importar `TipoEventoDetectado` lá criaria dependência Infrastructure → Domain que não existia. Violação de Dependency Inversion. `database.py` mantém porque já recebe `EventoDetectado` como parâmetro (estende dependência existente). `events.py` é wiring/bootstrap — mensagens cruzam fronteira de processo via outbox SQL onde strings são representação canônica. Princípio: refinar dependência existente é legítimo; criar dependência nova só por type safety cosmética não é. |
| `AlertaResolvido` verificado via outbox SQL, NÃO via campo no `RelatorioFonte` | `RelatorioFonte` representa o que aconteceu **a partir do batch coletado** — alertas resolvidos não vieram no batch (por definição, eles estão ausentes). Adicionar campo `resolvidos: int` misturaria dimensões. Resolvido é fenômeno **derivado** da ausência ao longo de N rodadas, observado pelo `ChangeDetector` cruzando snapshots ativos com batch corrente. Lugar correto para esse contador é endpoint de observabilidade dedicado da Camada 6 (`/eventos/recentes?tipo=AlertaResolvido`) ou métrica via Prometheus. Teste C do hardening-2 verifica via `json_extract(payload, '$.fonte')` e `json_extract(payload, '$.cod_alerta')` na tabela `eventos` — payload é contrato externo estável, schema interno pode mudar sem quebrar o teste. |
| `criar_banco()` chamado UMA VEZ no startup do scheduler, NÃO a cada rodada | Schema setup pertence ao bootstrap, não ao ciclo de execução. `agendar_ingestao()` chama `criar_banco()` antes de `scheduler.start()`, garantindo que erros de schema (incompatibilidade, migração necessária, `SchemaIncompativelError`) ocorram no startup — operador vê o erro imediatamente, não às 3h da manhã quando o primeiro tick rodar. Chamar a cada `_rodar_rodada` seria custo de I/O desnecessário (5 min × 365 dias = ~105k chamadas redundantes/ano). Padrão de indústria: Django roda `migrate` antes de `runserver`, SQLAlchemy roda `Base.metadata.create_all()` no startup, FastAPI lifespan events fazem setup pré-loop. Item 7 (DB_PATH explícito) eventual vai exigir que `criar_banco(db_path)` receba argumento, mas o ponto de chamada permanece o mesmo: startup, não loop. |
| `monitor.py` reduzido a entrypoint puro, lógica de orquestração centralizada no scheduler | Pré-B.2.b, `monitor.py` violava SRP três vezes: era entrypoint (`if __name__ == "__main__"`), continha lógica de orquestração (`executar_ingestao`), e tinha código morto de versão anterior (comparações de string com tipos de evento). Pós-B.2.b: `monitor.py` é 46 linhas com responsabilidade única — debug/desenvolvimento manual via `python -m alertavida.monitor` que executa UMA rodada. Produção orquestra via `agendar_ingestao()` em `scheduler.py`, que importa `executar_ingestao` do namespace canônico `alertavida.ingestion.orquestrador`. `monitor.py` e `scheduler.py` são os dois entrypoints do sistema; ambos chamam `criar_banco()` no setup e `executar_ingestao([CemadenSource()])` no fluxo de ingestão. Princípio: cada arquivo .py com nome técnico (monitor, scheduler) é um entrypoint específico, não um agregado de responsabilidades. |
| `main()` como função real em `monitor.py`, sem alias `main = executar_ingestao` | O alias antigo (`main = executar_ingestao` na linha 84 do `monitor.py` pré-B.2.b) servia para fazer `python -m alertavida.monitor` rodar a função de orquestração. Em B.2.b, `main()` virou função real com responsabilidades de entrypoint: chamar `criar_banco()` para bootstrap, instanciar source, chamar orquestrador, formatar e imprimir relatório, retornar exit code 0. Padrão idiomático Python (*Effective Python* item 41, *Modernizing Python Code*): entrypoints retornam `int` (exit code), são chamados via `raise SystemExit(main())` (não `sys.exit(main())` nem `exit(main())`), e separam lógica de I/O. `_formatar_relatorio` extraída como função pura (sem I/O) permite teste unitário sem mock. |
...
| 2026-05-12 | Pré-Parte B — desenho arquitetural da Camada 4 Parte B registrado antes da implementação. Decisões: `fonte` vira atributo do `Alerta` (consistência com proveniência da classificação A.2); `DataSource` ABC com `ResultadoColeta` tipado (preserva assertion sanitária por fonte); `FalhaDeColeta` exceção tipada; orquestrador isolado em `ingestion/orquestrador.py` (SRP — `monitor.py` viola três vezes); `RelatorioIngestao` tipado substitui contadores soltos; testes de contrato parametrizados em `tests/sources/contrato.py`; `FakeDataSource` em vez de `unittest.mock.Mock`; migração em três commits encadeados B.0 → B.1 → B.2 (big-bang rejeitado). CONTEXT.md atualizado antes do código. |
